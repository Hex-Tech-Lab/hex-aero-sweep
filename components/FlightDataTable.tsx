    ...
    >
      <TableCell className={cn('font-mono font-semibold text-[10px] px-1', isOutOfRange ? 'text-slate-400' : 'text-slate-100')}>
        <span title={carrierInfo?.name || CARRIER_NAMES[flight.carrier] || flight.carrier}>
          {carrierInfo?.logo_symbol_dark_url || carrierInfo?.logo_symbol_url ? (
            <img 
              src={carrierInfo.logo_symbol_dark_url || carrierInfo.logo_symbol_url!} 
              alt={flight.carrier} 
          ...
          {selectedFlight && (
            <div className="p-4 space-y-4 overflow-x-hidden">
              <div className="flex items-center gap-3 pb-3 border-b border-slate-800">
                {carrierCache.get(selectedFlight.carrier)?.logo_lockup_dark_url || carrierCache.get(selectedFlight.carrier)?.logo_lockup_url ? (
                  <img
                    src={carrierCache.get(selectedFlight.carrier)!.logo_lockup_dark_url || carrierCache.get(selectedFlight.carrier)!.logo_lockup_url!}
